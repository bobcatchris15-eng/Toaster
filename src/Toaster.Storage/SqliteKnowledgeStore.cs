...
""";
    ...
    public async Task<Observation> AddObservationAsync(Observation o, CancellationToken ct = default)
    {
        ...
        c.CommandText = "INSERT INTO observations VALUES($id,$a,$attempt,$result,$resolution,$env,$created)";
        c.Parameters.AddWithValue("$id", o.Id.ToString());
        c.Parameters.AddWithValue("$a", o.Activity);
        c.Parameters.AddWithValue("$attempt", o.Attempt);
        c.Parameters.AddWithValue("$result", o.Result);
        c.Parameters.AddWithValue("$resolution", (object?)o.Resolution ?? DBNull.Value);
        c.Parameters.AddWithValue("$env", (object?)o.EnvironmentJson ?? DBNull.Value);
        c.Parameters.AddWithValue("$created", o.CreatedAt.ToString("O"));
    ...
    public async Task<SourceDocument> AddSourceAsync(SourceDocument source, IReadOnlyList<SourceSection> sections, CancellationToken ct = default)
    {
        ...
        c.CommandText = "INSERT INTO sources VALUES($id,$title,$origin,$type,$sha,$at)";
        c.Parameters.AddWithValue("$id", source.Id.ToString());
        c.Parameters.AddWithValue("$title", source.Title);
        c.Parameters.AddWithValue("$origin", (object?)source.Origin ?? DBNull.Value);
        c.Parameters.AddWithValue("$type", source.ContentType);
        c.Parameters.AddWithValue("$sha", source.Sha256);
        c.Parameters.AddWithValue("$at", source.ImportedAt.ToString("O"));
        await c.ExecuteNonQueryAsync(ct);
        foreach (var s in sections)
        {
            var sc = db.CreateCommand();
            sc.Transaction = (SqliteTransaction)tx;
            sc.CommandText = "INSERT INTO source_sections VALUES($id,$source,$heading,$content,$ordinal)";
            sc.Parameters.AddWithValue("$id", s.Id.ToString()); sc.Parameters.AddWithValue("$source", s.SourceId.ToString()); sc.Parameters.AddWithValue("$heading", s.Heading); sc.Parameters.AddWithValue("$content", s.Content); sc.Parameters.AddWithValue("$ordinal", s.Ordinal);
            await sc.ExecuteNonQueryAsync(ct);
            var f = db.CreateCommand();
            f.Transaction = (SqliteTransaction)tx;
            f.CommandText = "INSERT INTO source_fts(id,source_id,heading,content) VALUES($id,$source,$heading,$content)";
            f.Parameters.AddWithValue("$id", s.Id.ToString()); f.Parameters.AddWithValue("$source", s.SourceId.ToString()); f.Parameters.AddWithValue("$heading", s.Heading); f.Parameters.AddWithValue("$content", s.Content);
    ...
    public async Task<IReadOnlyList<SourcePreview>> SearchSourcesAsync(string query, int limit, CancellationToken ct = default)
    {
        ...
        c.Parameters.AddWithValue("$q", ToFtsQuery(query));
        c.Parameters.AddWithValue("$limit", Math.Clamp(limit, 1, 50));
        var x = new List<SourcePreview>();
        await using var r = await c.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct)) x.Add(new SourcePreview(Guid.Parse(r.GetString(0)), r.GetString(1), r.IsDBNull(2) ? null : r.GetString(2), r.GetString(3), r.GetString(4)));
        return x;
    }
